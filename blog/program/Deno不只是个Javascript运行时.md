...
```powershell
deno run https://deno.wenhao.cn/main.ts
```

在第一次使用时下载并缓存代码，你可以通过

```powershell
deno info http://deno.wenhao.cn/main.ts
```
...
你可以通过 [https://wenhao.deno.dev/](https://wenhao.deno.dev/ 'https://wenhao.deno.dev/') 来访问我使用 deno Deploy 所创建的一个在线项目。将会输出一个`Hello World!` 的页面。
