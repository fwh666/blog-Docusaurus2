...
![](https://img.wenhao.cn/link-maker.png)
...
![](https://img.wenhao.cn/image_jSRfPu966v.png)
...
![](https://img.wenhao.cn/image_v73eXB47yI.png)
...
核心代码在 [`routes\api\link.ts`](https://github.com/wenhao/link-maker/blob/main/routes/api/link.ts) 下，将会生成 `/api/link` 接口，例如访问 [https://link-maker.deno.dev/api/link?q=https://wenhao.cn](https://link-maker.deno.dev/api/link?q=https://wenhao.cn 'https://link-maker.deno.dev/api/link?q=https://wenhao.cn') 你就可以得到如下 json 数据
...
至于说为何要用 query 而不是用 param，主要是因为 url 的缘故，比如说 `/api/link/https://wenhao.cn` 这个链接，这时 param 是解析不出 `https://wenhao.cn` 完整 url 的，除非url编码，但这对使用者来说就不是很好，于是就舍弃了 param 参数的方案。
...
![](https://img.wenhao.cn/image_CYOAgv6IGe.png)
