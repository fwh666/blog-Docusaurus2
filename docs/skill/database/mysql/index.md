...
-- 修改用户名与命名
UPDATE `user` SET `username`='wenhao',`password`='a12345678` WHERE id = 1
```
...
![img](https://img.wenhao.cn/20201009150524563.png)
...
SELECT LOWER('wenhao') --到小写
SELECT UPPER('wenhao') --到大写
```
...
CREATE USER wenhao IDENTIFIED BY '123456'

-- 修改密码 (修改当前用户)
SET PASSWORD = PASSWORD('a123456')

-- 修改密码 (修改指定用户)
SET PASSWORD FOR wenhao= PASSWORD('a123456')

-- 重命名
RENAME USER wenhao TO zeyu

-- 用户授权
-- CRANT ALL PRIVILEGES ON 库.表 TO 用户
-- ALL PRIVILEGES 除了给别人授权(GRANT),其他权限都有
CRANT ALL PRIVILEGES ON *.* TO wenhao
...
REVOKE ALL PRIVILEGES ON *.* FROM wenhao
...
![image-20210102213536190](https://img.wenhao.cn/image-20210102213536190.png)
